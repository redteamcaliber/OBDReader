/*
    ...
    Description: A handler that handles the
                 messages sent by the threads
                 of the OBDReader app.
*/
...
// Custom handler that handles the exchange of
// the read mBuffer between the main thread and
// the read thread. Prevents memory leak using
...
public class ThreadHandler extends Handler {
    ...
    // Denotes that the message was read
    // by the handler.
    private final int MESSAGE_READ = 1;

    // Denotes that the Bluetooth connection
    ...
    // Handles the messages sent by the
    // read thread of the app.
    @Override
